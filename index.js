...
app.get('/api/admin/tokens', (req, res) => {
  ...
  res.json({
    ...
    payWeekStart: process.env.PAY_WEEK_START || 'MON',
    payCutoffHHMM: process.env.PAY_CUTOFF_HHMM || '1700',
    tokens: map
  });
});
...
// ---------- Fetch logs (privacy enforced at UI level) ----------
...
app.post('/api/period/close', async (req, res) => {
  ...
  try { fs.writeFileSync(fname, csv, 'utf8'); } catch (e) { console.error('save CSV failed:', e.message); }
